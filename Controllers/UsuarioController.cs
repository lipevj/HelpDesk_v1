    ...
    public class UsuarioController : Controller
    {
        ...
        public RoleManager<IdentityRole> RoleManager
        {
            get
            {
                ...
                {
                    var contextoOwin = HttpContext.GetOwinContext();
                    _roleManager = contextoOwin.GetUserManager<RoleManager<IdentityRole>>();
                }
        ...
        public async Task<ActionResult> EditarFuncoes(UsuarioEditarFuncoesViewModel modelo)
        {
            if (ModelState.IsValid)
            {
                ...
                var resultadoRemocao = await UserManager.RemoveFromRolesAsync(
                   usuario.Id, rolesUsuario.ToArray());

                if (resultadoRemocao.Succeeded)
                {
                    var funcoesSelecionadasPeloAdmin = modelo.Funcoes.Where
                        (funcao => funcao.Selecionado).Select(funcao => funcao.Nome).ToArray();

                    var resultadoAdicao = await UserManager.AddToRolesAsync(usuario.Id,
